"""Plot the sectional loads for the XV-15 rotor quick-start case"""
...
loads = ["blade_CFz_per_span", "blade_CFx_per_span"]
figures = []
axes = []


def plot_sectional_forces(data, plot_name):
    """Plot the sectional loads"""
    for ax, load in zip(axes, loads):
        ax.plot(data["Y"], data["innerRotating/" + load], label=plot_name)
        ax.set_ylabel(load)
        ax.legend()
        ax.set_xlabel("Y")
